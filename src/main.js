...
(async (root) => {
  ...
  page('/cart', async () => {
    ...
    const cartRessources = await getCartRessources(1);
    for (const cartRessource of cartRessources) {
      cartRessource.product = await getProduct(cartRessource.productId);
    }

    AppCart.products = cartRessources;
